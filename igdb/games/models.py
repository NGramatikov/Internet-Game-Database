...
'''
First we define the constants: the game types, minimum age range, release year (the first video game- "Tennis for Two" 
was released in 1958) and how much chance is involved for games with dice or cards.
'''
...
'''
We create the parent class for all games. If we don't set the default type to Video Games we will get a 
{'type': ['This field cannot be blank.']}" error when trying to create a VideoGame object. It didn't matter if we set
the type to initial="Video Games" in the form. If we try to ser models.Model before the other classe we will get a
TypeError: Cannot create a consistent method resolution order (MRO) and there's hardly any point in repeating ourselves
by typing all the interaction classes in both children of Game.
'''


class Game(Likeable, Rateable, Commentable, Reviewable, models.Model):
    class Meta:
        abstract = True
